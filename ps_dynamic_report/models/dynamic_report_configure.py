    ...
    def create_dynamic_pdf_report(self, server):
        ...
        for model in active_model_id:
            ...
            for item in dynamic_report_id:
                ...
                if item.field_type == 'selection':
                    field = model._fields.get(item.field_name_id)
                    if field:
                        selection = field.selection
                        if callable(selection):
                            selection = selection(model)  # assuming model is a single record
                        state = dict(selection).get(model[item.field_name_id], model[item.field_name_id])
                        temp.append(state)
